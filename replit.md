...
- **Gallery Array Parsing**: Both the server (`server/storage.ts`) and CF Workers (`functions/_lib/db.ts`) register a neon type parser for PostgreSQL text array columns (OID 1009) using the shared `parseTextArray` utility in `shared/parseTextArray.ts`. This handles the `{val1,val2}` format natively at the driver level, eliminating the need for post-query fixup functions.
