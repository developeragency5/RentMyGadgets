...
const STATIC_ROUTES: Record<string, PageMeta> = {
  ...
  "/search": {
    ...
    description: "Search our full catalog of over 130 rental tech products. Find laptops, cameras, desktops, and accessories available for flexible monthly rent.",
  ...
  "/how-it-works": {
    ...
    description: "Learn how to rent technology equipment from RentMyGadgets. Browse, select your rental period, and get fast delivery in select areas. Simple returns included.",
    jsonLd: { "@context": "https://schema.org", "@type": "HowTo", name: "How to Rent Technology Equipment", description: "A step-by-step guide to renting premium tech equipment from RentMyGadgets.", url: `${BASE_URL}/how-it-works`, step: [{ "@type": "HowToStep", name: "Browse Products", text: "Browse our catalog of over 130 products or explore by category to find the equipment you need." }, { "@type": "HowToStep", name: "Choose Rental Period", text: "Select your rental period from 1, 3, 6, or 12 months with progressive discounts for longer terms." }, { "@type": "HowToStep", name: "Receive and Use", text: "We ship your equipment with fast delivery in select areas available. Use it for your rental period and return with our prepaid label." }] },
  },
  "/gadgetcare": {
...
async function getCategoryMeta(env: Env, categoryId: string): Promise<PageMeta | null> {
  try {
    const db = getDb(env);
...
export async function getMetaForUrl(env: Env, url: string): Promise<PageMeta> {
  ...
  if (productMatch) {
    const meta = await getProductMeta(env, productMatch[1]);
    if (meta) return meta;
  }

  const categoryMatch = cleanUrl.match(/^\/categories\/([^/]+)$/);
  if (categoryMatch) {
    const meta = await getCategoryMeta(env, categoryMatch[1]);
    if (meta) return meta;
  }
...
<h2>What We Offer</h2>
<p>Our <a href="/products">product catalog</a> features over 130 items across multiple <a href="/categories">categories</a> including laptops, desktops, cameras, audio equipment, accessories, and networking gear. Brands include Dell, HP, Lenovo, Samsung, Canon, Sony, Nikon, and more. Every rental can be enhanced with optional <a href="/gadgetcare">GadgetCare+ protection</a> for coverage against accidental damage, spills, and hardware malfunctions. Our <a href="/rent-to-own">Rent-to-Own program</a> lets you purchase equipment after 6 months at a reduced buyout price. Learn more about how the process works on our <a href="/how-it-works">How It Works</a> page, read our <a href="/blog">blog</a> for rental tips and guides, or <a href="/contact">contact our team</a> with questions about your rental needs.</p>`,
...
<p>Step 1: <a href="/products">Browse our catalog</a> of over 130 products or <a href="/categories">explore by category</a>. Find laptops, desktops, cameras, audio equipment, and accessories from brands like Dell, HP, Canon, and Sony. Use our <a href="/search">search tool</a> or <a href="/compare">comparison feature</a> to find exactly what you need.</p>
...
<p>Ready to get started? <a href="/products">Browse our full product catalog</a> of over 130 rental items, or <a href="/categories">explore by category</a> to find laptops, cameras, desktops, and more. Learn about our <a href="/how-it-works">rental process</a>, <a href="/gadgetcare">GadgetCare+ protection</a>, or <a href="/rent-to-own">Rent-to-Own program</a>. <a href="/search">Search for specific products</a> or <a href="/compare">compare items</a> side by side. <a href="/contact">Contact our team</a> with questions.</p>`,
  ...
  "/search": `<p>Search the complete RentMyGadgets catalog to find the perfect technology equipment for your rental needs. Our catalog includes over 130 products across all major categories including laptops, desktops, cameras, audio equipment, networking gear, and accessories from brands like Dell, HP, Lenovo, Samsung, Canon, Sony, and Nikon.</p>
<h2>Find the Right Rental Equipment</h2>
<p>Use the search bar above to find specific products by name, brand, or type. Looking for a Dell XPS? Type the model name to see all available configurations and rental pricing. Need a camera for a weekend shoot? Search for Canon, Sony, or Nikon to see our full camera selection. You can also filter search results by category, price range, and availability to narrow down your options quickly.</p>
...
async function getCrawlerNav(env: Env, url: string): Promise<CrawlerNavParts> {
  ...
  try {
    ...
    const blogLinks = allBlogPosts
      ...
      .map(p => `<a href="/blog/${escapeHtml(p.slug)}">${escapeHtml(p.title)}</a>`);
